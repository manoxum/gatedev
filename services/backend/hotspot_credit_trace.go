...
func openCreditTrace(ctx context.Context, mongoClient *mongo.Client) (*creditTraceClient, error) {
	collection := mongoClient.Database(getenv("MONGO_DB", "bindnet")).Collection("hotspot_credit_debits")
...
func creditTraceRetentionSeconds() int32 {
	days, err := strconv.Atoi(getenv("HOTSPOT_CREDIT_TRACE_RETENTION_DAYS", "180"))
	if err != nil || days <= 0 {
		days = 180
	}
