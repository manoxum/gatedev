...
func registerHotspotDeviceQuotaRoutes(mux *http.ServeMux, admin *administrator, db *sql.DB, worker *workerClient, audit *auditClient) {
	mux.HandleFunc("GET /api/hotspot/devices/{mac}/quota", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("POST /api/hotspot/devices/{mac}/quota/{period}/reset", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
		mac, err := normalizeHotspotMAC(r.PathValue("mac"))
		if err != nil {
			http.Error(w, "mac invalido", http.StatusBadRequest)
			return
