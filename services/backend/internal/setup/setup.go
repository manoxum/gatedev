...
func RegisterSetupRoutes(mux *http.ServeMux, admin *auth.Administrator, db *sql.DB, audit *audit.Client) {
	mux.HandleFunc("GET /api/setup/status", auth.RequireSession(admin, func(w http.ResponseWriter, r *http.Request) {
...
func pingMongo(ctx context.Context, audit *audit.Client) setupServiceStatus {
	if audit == nil {
...
func pingMinio(ctx context.Context) setupServiceStatus {
	host := config.Getenv("MINIO_HOST", "minio")
	port := config.Getenv("MINIO_PORT", "9000")
	dialer := net.Dialer{}
