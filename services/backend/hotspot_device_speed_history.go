...
func registerHotspotDeviceSpeedHistoryRoutes(mux *http.ServeMux, admin *administrator, speedHistory *deviceSpeedHistoryStore) {
	mux.HandleFunc("GET /api/hotspot/devices/{mac}/speed-history", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("GET /api/hotspot/limits/global/speed-history", requireSession(admin, func(w http.ResponseWriter, r *http.Request) {
		minutes := 15
		if raw := r.URL.Query().Get("minutes"); raw != "" {
			parsed, convErr := parseSpeedHistoryWindow(raw)
			if convErr != nil {
