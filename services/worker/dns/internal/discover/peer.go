// Package discover implementa o "discover mode": troca de vetor de
// distancia entre servidores Bindnet vizinhos (via HTTP), calculo das
// rotas proprias deste no e a tabela de rotas aprendida da malha.
package discover
...
// essencial pra reconstruir o vetor de distancia do lado de quem recebe;
// "nextHop"/"source"/"state" sao sempre recalculados localmente por quem
// recebe, nunca repassados como vieram.
...
func ownRoutes(cfg *core.Config) map[string]core.Route {
	owned := map[string]core.Route{}
	now := time.Now()

	addOwned := func(name string) {
		owned[name] = core.Route{
			Domain:           name,
			Owner:            cfg.NodeName,
	...
	addIfInMesh := func(name string) {
		if _, ok := zones.SuffixZoneFor(strings.Split(name, "."), cfg.DomainZones); !ok {
			return
		}
		addOwned(name)
	}
	for zone := range cfg.DomainZones {
		if zones.IsConcreteOwnedDomainZone(zone) {
			addOwned(zone)
		}
	}
...
// horizon - nunca devolve pra um vizinho uma rota que so existe porque foi
// ele quem a anunciou).
func advertisedRoutesFor(cfg *core.Config, requesterIP string) []advertisedRoute {
	var result []advertisedRoute
	seen := map[string]bool{}
	add := func(route core.Route) {
...
// hostOf extrai o IP de um endereco "host:porta"; devolve o valor original
// se nao tiver esse formato (ex.: a origem sintetica "self", que nunca
// deve bater com um IP de requisitante de verdade).
...
// StartServer expoe a tabela de rotas deste no para os peers - unico
// servidor HTTP do dns-provider, que ate aqui so tinha sockets UDP:53.
// Sobe mesmo sem peer direto configurado, porque um no "folha" ainda
// precisa responder quando outro no o lista como vizinho.
func StartServer(cfg *core.Config, port string) {
...
func routeDomains(routes map[string]core.Route) []string {
